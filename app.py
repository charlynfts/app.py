...
def analyze_with_claude(frames_base64, api_key, model="claude-3-5-sonnet-20241022"):
...
# Sidebar - Configuración (solo Secrets en la nube)
with st.sidebar:
    st.header("🔑 Configuración")
    
    # Lee directamente del secreto de Streamlit Cloud
    api_key = st.secrets.get("ANTHROPIC_API_KEY")
    
    if not api_key:
        st.error("No se encontró la Anthropic API Key.\n\n"
                 "Andá a Manage app → Secrets y agregá:\n"
                 'ANTHROPIC_API_KEY = "sk-ant-..."')
        st.stop()
    
    model = st.selectbox("Modelo Claude", ["claude-3-5-sonnet-20241022", "claude-3-opus-20240229"], index=0)
    st.info("Sonnet es más rápido y barato. Opus para máxima precisión (más caro).")

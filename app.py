...
def extract_frames(video_path, num_frames=8):
    """Extrae frames uniformes del video y los guarda + devuelve base64"""
...
def analyze_with_claude(frames_base64, api_key, model="claude-sonnet-4-6"):
...
def search_items(query, api_key):
    """Búsqueda inteligente con Claude usando todo el historial"""
    items = get_all_items()
    if not items:
...
with st.sidebar:
    ...
    api_key = st.text_input("Anthropic API Key", type="password", help="Conseguila en https://console.anthropic.com/")
    model = st.selectbox("Modelo Claude", ["claude-sonnet-4-6", "claude-opus-4-6"], index=0)
    st.info("Sonnet 4.6 es más rápido y barato. Opus para máxima precisión.")
    
    if st.button("Limpiar base de datos (cuidado)"):
        if st.checkbox("¿Estás seguro? Se borra todo"):
            os.system("rm -rf donde_carajo.db videos frames")
            st.success("Todo limpio. Reiniciá la app.")
            st.rerun()
...
with tab1:
    ...
    if uploaded_video and api_key and st.button("🚀 Catalogar ahora", type="primary"):
        with st.spinner("Procesando video + mandando a Claude... esto puede tardar 15-30 seg"):
            # Guardar video
            ...
            # Analizar con Claude
            analysis = analyze_with_claude(frames_b64, api_key, model)
            
            # Guardar en DB
            st.session_state.current_catalog_id = save_catalog(video_save_path, analysis)
            
            st.success("✅ Catalogado con éxito!")
            st.json(analysis)  # Mostrar resultado crudo
            
            # Mostrar frames guardados
...
with tab2:
    ...
    if search_query and api_key and st.button("Buscar con Claude"):
        with st.spinner("Claude está revisando TODOS tus catálogos..."):
            result = search_items(search_query, api_key)
            st.markdown(result)
            
            # Bonus: mostrar imágenes si menciona paths
...
with tab4:
    ...
    if len(catalogs) < 1:
        ...
        if new_video and api_key and st.button("Comparar y ver qué desapareció"):
            with st.spinner("Analizando diferencia..."):
                # Guardar nuevo video temporal
                with tempfile.NamedTemporaryFile(delete=False, suffix=".mp4") as tmp:
                    tmp.write(new_video.getvalue())
                    temp_path = tmp.name
                
                frames_b64, _ = extract_frames(temp_path, 8)
                new_analysis = analyze_with_claude(frames_b64, api_key, model)
